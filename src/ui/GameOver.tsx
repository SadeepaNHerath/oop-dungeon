...
export function GameOver() {
  ...
  const reason = state.lastFeedback?.wrongReason ?? state.lastFeedback?.hint
    ...
    <main className="mx-auto max-w-lg px-4 py-16 text-center fade-up">
      ...
      {room ? (
        <p className="mt-3 text-faded">
          You stalled on <span className="text-parchment">{room.name}</span>.
        </p>
      ) : null}
      {reason ? (
        <p className="mt-6 rounded-lg border border-edge bg-panel px-4 py-3 text-left text-sm text-parchment">
          <span className="font-semibold text-rune">What went wrong. </span>
          {reason}
        </p>
      ) : null}
      {trap ? (
      ...
      <p className="mt-4 text-xs text-faded">
        Retry restores HP. Levels you already cleared stay cleared.
      </p>
    </main>
  )
}
